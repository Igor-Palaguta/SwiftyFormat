...
final class SwiftyFormatTests: XCTestCase {
    private let format = "Hello #{{name|Mr(s)}}, I have #{{cookies|several}} cookies for you. Bye #{{name}}"
    ...
    func testAttributedMapping() {
        let name = NSAttributedString(string: "Jack", attributes: [.underlineStyle: 1])
        let cookies = NSAttributedString(string: "100", attributes: [.underlineStyle: 1])
        let result = NSAttributedString(format: format) { key in
            switch key {
            case "name":
                return name
    ...
    func testString() {
        ...
        func testAttributes() {
            ...
            expectedNameAttributes[NSAttributedString.Key.underlineStyle] = 1 as Any
